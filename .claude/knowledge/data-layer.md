...
| `EmailAccount`   | `email_accounts`   | One connected Gmail inbox (`provider` currently only `"GMAIL"`). Holds OAuth `accessToken`/`refreshToken`/`tokenExpiry` **directly in SQLite**, written by the OAuth callback route — see the exception to hard rule 1 in `CLAUDE.md` and the gotcha below.                                                                                                                                                                                                                                                                                                    |
| `JobEmail`       | `job_emails`       | One synced recruiting email. Optional FKs to `Job`/`Company` (`onDelete: SetNull`, so deleting the job keeps the email as unmatched rather than deleting it); `jobMatcher.ts` does the matching. `stage`/`confidence`/`nextSteps`/`actionRequired` come from `src/lib/llm/emailClassifier.ts` (client-side LLM call, consistent with hard rule 1 — see [llm-runtime.md](llm-runtime.md)).                                                                                                                                                                    |
...
| `emailSync.ts`    | `getEmailSyncStatus`, `syncEmails`, connect/disconnect and custom-OAuth-client-credential actions backing `EmailTrackingSettings.tsx`/`useEmailSync.ts`. Reads/writes `EmailAccount`/`JobEmail`. **Gotcha:** calls `src/lib/email/gmailClient.ts`'s `getValidAccessToken()`, which reads cached tokens through `keyStorage.getApiKey` — that function's non-Tauri fallback calls browser `localStorage`, which doesn't exist in a `'use server'` action's Node runtime. The call throws internally, is swallowed, and returns `null`. Token refresh from a server action is effectively dead code; only the initial OAuth connect (the callback route writing straight to `EmailAccount` via Prisma) persists a usable token. Not fixed as of this doc pass — flagged, not repaired. |
