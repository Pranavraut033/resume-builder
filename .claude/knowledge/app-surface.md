...
| `/settings`          | `settings/page.tsx`                   | API keys (add/remove per provider), model selection, MCP server toggle + connector download, Backup & Restore, email tracking (`EmailTrackingSettings.tsx` — connect a self-provisioned Google OAuth client, see [docs/EMAIL_TRACKING.md](../../docs/EMAIL_TRACKING.md); see [data-layer.md](data-layer.md) for `EmailAccount`/`JobEmail`). A "What's New" modal (`WhatsNewModal.tsx`, gated once-per-version by `WhatsNewGate.tsx`/`versionFlag.ts`) replaced the old CHANGELOG-on-GitHub link, fed by `releaseNotes.generated.ts` (built from `CHANGELOG.md` by `scripts/gen-release-notes.mjs`, run in `predev`/`prebuild`). |
