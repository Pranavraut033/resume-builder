...
import { parseJobDescription, generateResume, generateCoverLetter, ProviderFactory } from '@/lib/clientLLM';
import { sampleBaseProfile, sampleJobDetails, sampleTailoredResume } from '../fixtures/data';
...
describe('Client LLM Functions', () => {
  ...
  describe('parseJobDescription', () => {
    it('should parse job description using OpenAI provider', async () => {
      ...
      expect(result).toEqual({ ...sampleJobDetails, raw_description: 'Job description text' });
      expect(mockParseJobDetails).toHaveBeenCalledWith('Job description text', 'gpt-4o');
    });
  ...
  describe('generateResume', () => {
    ...
    it('should throw error when no provider is available', async () => {
      ...
      await expect(
        generateResume(sampleBaseProfile, sampleJobDetails.raw_description, sampleJobDetails.job.job_title, sampleJobDetails.company.company_name, 'model', 'openai')
    ...
    it('should propagate provider errors', async () => {
      ...
      await expect(
        generateResume(sampleBaseProfile, sampleJobDetails.raw_description, sampleJobDetails.job.job_title, sampleJobDetails.company.company_name, 'gpt-4o', 'openai')
  ...
  describe('ProviderFactory', () => {
    it('should cache provider instances', async () => {
      ...
      await generateResume(sampleBaseProfile, sampleJobDetails.raw_description, sampleJobDetails.job.job_title, sampleJobDetails.company.company_name, 'gpt-4o', 'openai');
      await generateResume(sampleBaseProfile, sampleJobDetails.raw_description, sampleJobDetails.job.job_title, sampleJobDetails.company.company_name, 'gpt-4o', 'openai');
