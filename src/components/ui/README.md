...
const PrimaryButton = ({ children, className = '', ...props }) => (
  <button className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary-500 text-white shadow ${className}`} {...props}>{children}</button>
)
```
...
const Card = ({ children, className = '' }) => (
  <div className={`p-6 bg-white rounded-xl shadow-subtle ${className}`}>{children}</div>
)
```
