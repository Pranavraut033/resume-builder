...
export function MultiSelect({
  ...
  return (
    <div className={`relative ${className}`}>
      <Combobox value={null} onChange={(val) => val && !disabled && toggleOption(val)} disabled={disabled}>
        <div className="relative">
          <div className={`w-full border border-gray-300 dark:border-gray-600 rounded-lg px-2 py-1 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
            <div className="flex flex-wrap gap-1 items-center">
              {value.map(v => (
                <span key={v} className="flex items-center bg-gray-100 dark:bg-gray-600 text-sm rounded-full px-2 py-0.5">
                  ...
                  {({ selected, active }) => (
                    <>
                      ...
                      <span className={`absolute inset-y-0 left-0 flex items-center pl-3 ${active ? 'text-white' : 'text-primary-600'}`}>
                        {value.includes(option) ? <Icon name="check" size={16} /> : null}
                      </span>
                    </>
                  )}
                </Combobox.Option>
