...
2. Does the message ask to check, review, scan, or proofread the resume for typos, spelling mistakes, grammar issues, formatting inconsistencies, duplicated bullets, or other mistakes — whether phrased as a question ("any typos in my resume?", "does my resume have mistakes?") or an instruction ("proofread my resume", "check my resume for typos")? → proofread. This wins over question (check 3) and edit (check 6) even though it is often phrased as a question or names a section.
3. Is the user asking a question (starts with or contains "what", "should I", "am I", "how do I", "will they", "is this", "can I") rather than instructing a change? → question. This wins even if a section name is mentioned, e.g. "what more can I add to my experience section" is question, not edit — the user hasn't said to change anything yet. But hedging words like "not sure", "maybe", "I guess" do NOT make a message a question by themselves — if the message still names a concrete action to take ("fix summary", "redo it"), classify by that instruction (edit/regenerate/etc.), not question.
4. Does the message ask specifically for the cover letter to sound less AI-generated, more natural, less robotic, or less like a template ("sounds like AI wrote it", "too robotic", "sound more human", "make it less generic-sounding")? → humanize. This wins over cover_letter whenever the complaint is about naturalness/AI-detection rather than content or tone.
5. Does the message name or reference the cover letter, and instruct or request a change to its content, tone, or style (that isn't specifically about sounding-more-natural/less-AI)? → cover_letter
6. Does the message name or reference a specific resume section, and instruct or request a change to it (even negatively, e.g. "I don't like my summary")? → edit
7. Does the message ask for interview prep, likely questions, or how to answer something? → interview
8. Does the message say "redo", "rewrite", "start over", or "new resume" for the WHOLE resume (not one section)? → regenerate
9. Does the message ask to adjust the whole resume to fit the job description, without naming one section? → tailor
10. Does the message instruct the ATS issues/findings/suggestions themselves to be fixed, applied, or resolved ("fix my ATS issues", "apply all ATS suggestions", "resolve the ATS findings", "fix all the ATS problems")? → fix_ats
11. Does the message ask specifically about ATS or applicant tracking systems, without instructing a fix (e.g. asking for a score, an explanation, or "how do I improve")? → ats
12. None of the above clearly apply → other
...
Respond with ONLY the intent label as a single word from "edit", "interview", "tailor", "regenerate", "question", "ats", "fix_ats", "proofread", "cover_letter", "humanize", "undo", or "other".
