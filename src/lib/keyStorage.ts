...
async function loadKeyStore(): Promise<KeyStore> {
  ...
  try {
    ...
    logger.info('Key store loaded from disk', { keyCount: keyStore ? Object.keys(keyStore).length : 0 });
    return keyStore!;
  } catch (error) {
 ...
 */
async function decrypt(encryptedData: EncryptedData, password: string): Promise<string> {
...
export async function setApiKey(provider: string, apiKey: string): Promise<void> {
  if (!isTauriContext()) {
    logger.warn('Not in Tauri context, using localStorage fallback', { provider });
    ...
    try {
      ...
      logger.error(`Failed to store API key in localStorage for ${provider}`, { error, provider });
  ...
  try {
    ...
    logger.error(`Failed to store API key for ${provider}`, { error, provider });
    throw new Error(`Failed to store API key: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getApiKey(provider: string): Promise<string | null> {
  if (!isTauriContext()) {
    ...
    try {
      ...
      logger.debug(key ? 'API key retrieved from localStorage (web mode)' : 'API key not found in localStorage', { provider });
      return key;
    } catch (error) {
      logger.error('Failed to retrieve API key from localStorage', { provider, error });
      return null;
    }
  }
...
export async function deleteApiKey(provider: string): Promise<void> {
  if (!isTauriContext()) {
    ...
    try {
      ...
      logger.error(`Failed to delete API key from localStorage for ${provider}`, { error, provider });
  ...
  try {
    ...
    logger.error(`Failed to delete API key for ${provider}`, { error, provider });
    throw new Error(`Failed to delete API key: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
