...
const REPO_BLOB_BASE =
  "https://github.com/Pranavraut033/resume-builder/blob/main";
...
function absolutizeRelativeLinks(markdown: string, sourceFile: string): string {
  ...
  return markdown.replace(
    /(!?\]\()(?!https?:\/\/|#|mailto:)([^)\s]+)(\))/g,
    (_match, open, link, close) => {
      const resolved = new URL(
        link,
        `${REPO_BLOB_BASE}/${sourceDir}/`
      ).toString();
      ...
      for (const source of SOURCES) {
        try {
          ...
          const section = source.section
            ? extractSection(raw, source.section)
            : raw;
          const markdown = absolutizeRelativeLinks(
            stripLeadingHeading(section),
            source.file
          );
          ...
          logger.error(
            `docs-loader: failed to load ${source.file}: ${(err as Error).message}`
          );
        }
      }
    },
